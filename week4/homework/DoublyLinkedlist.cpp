...
void removeTail(List*& L)
{
    if (L->p_head)
    {
        if (L->p_tail->p_prev != nullptr)
        {
            NODE* p = L->p_tail->p_prev;
            p->p_next = nullptr;
            NODE* temp = L->p_tail;
            L->p_tail = p;
            delete temp;
...
void removePos(List*& L, int data, int pos)
{
    ...
    if (pos == 0)
    {
        ...
        if (L->p_head->p_next &&L->p_head->key==data)
        {
            NODE* p = L->p_head;
            L->p_head = p->p_next;
            L->p_head->p_prev = nullptr;
    ...
    while (p->p_next)
    {
        if (dem == pos - 1&&p->p_next->key==data)
        {
...
bool addAfter(List*& L, int data, int val)
{
    if (L->p_head->key == val)
    {
        ...
        if (L->p_head->p_next)
        {
            ...
            temp->p_prev = L->p_head;
            L->p_head->p_next->p_prev = temp;
            L->p_head->p_next = temp;
        }
        else
        {
            L->p_head = temp;
...
List* reverseList(List* L)
{
    ...
    result->p_head = nullptr;
    result->p_tail = nullptr;
    for (NODE* p = L->p_tail; p != nullptr; p = p->p_prev)
    {
        NODE* temp = createNode(p->key);
        if (result->p_head == nullptr)
        {
            result->p_head = temp;
            result->p_tail = temp;
        }
        ...
        {
            result->p_tail->p_next = temp;
            temp->p_prev = result->p_tail;
            result->p_tail = temp;
        }
     }
    return result;
